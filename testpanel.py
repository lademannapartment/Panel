...
# --- ЭКРАН 1: Приветствие (Логотип, синий фон и кнопка Старт) ---
if not st.session_state["started"]:
  # Задаем красивый синий фон под цвет логотипа (#0e3b66 — замените на ваш точный HEX код, если нужно)
  st.markdown(
      """
        <style>
        .stApp {
        ...
        /* Центрируем элементы первого экрана */
        .centered-content {
            display: flex;
  ...
  # Вывод логотипа по центру (файл 1.png)
  ...
  with col2:
    ...
    # Кнопка СТАРТ
    if st.button("🚀 Старт", use_container_width=True, type="primary"):
  ...
  if not st.session_state["authenticated"]:
    ...
    if st.sidebar.button("Wyloguj się"):
      ...
      st.session_state["started"] = (
          False  # При выходе можно возвращать на экран со стартом
      )
      st.rerun()
